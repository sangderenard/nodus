...
static inline float inv_relu(float x) { return x > 0.0f ? x : 0.0f; }
static inline float inv_sigmoid(float x) {
    const float eps = 1.0e-7f;
    float y = x;
    if (y < eps) y = eps;
    if (y > 1.0f - eps) y = 1.0f - eps;
    return logf(y / (1.0f - y));
}
static inline float inv_tanh(float x) {
    const float eps = 1.0e-7f;
    float y = x;
    if (y < -1.0f + eps) y = -1.0f + eps;
    if (y > 1.0f - eps) y = 1.0f - eps;
    return 0.5f * logf((1.0f + y) / (1.0f - y));
}
