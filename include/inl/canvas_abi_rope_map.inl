        ...
        if (r.kind == ModuleRowKind::Tool && r.tool_origin == ModuleToolOrigin::Plugin && !r.plugin_id.empty()) {
            ...
            try {
                ...
                if (inst) {
                    ...
                    // A named plugin row with no registered factory is a real
                    // configuration fault, not a quiet default -- say so once
                    // (see the boundary error register: silent null create()
                    // cost a full debugging campaign headlessly).
                    fprintf(stderr, "canvas_apply_io_rows: no registered tool for plugin_id='%s' (module=%d row=%zu)\n",
                        r.plugin_id.c_str(), module_idx, ri);
                }
            } catch (...) {}
        }
    }
